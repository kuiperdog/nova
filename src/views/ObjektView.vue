...
<template>
    <div id="objektView">
        ...
        <div id="filters">
            <Dropdown @valueChanged="x => update('artist', x)" :value="$route.query.artist" :options="[ {value: '', label: 'Any Artist', icon: defaultIcon}, ...artists ]"/>
...
export default {
    ...
    methods: {
        ...
        async getArtists() {
            const artistRes = await fetch(this.COSMO_API + '/artist/v1')
            ...
            for (const artist of artists) {
                ...
                const memberRes = await fetch(this.COSMO_API + '/artist/v1/' + artist.name)
                const members = (await memberRes.json()).artist.members
                this.artists.push(...members.map(member => Object({
                    value: member.name,
                    label: member.name,
